...
{
    private float getIntervalSize {
        get {
            return _grid.cellSize.x + _grid.spacing.x;
        }
    }
...
#region OnBeginDrag関数、OnEndDrag関数の実装
    ...
    public System.Action<int> mCbEndPaging;
	...
	void LateUpdate()
	{
		...
		if(isAnimating)
		{
			if(Time.time >= animationCurve.keys[animationCurve.length-1].time)
			{
                ...
                if(mCbEndPaging != null)
                {
                    mCbEndPaging(prevPageIndex);
                }
                return;
			}
